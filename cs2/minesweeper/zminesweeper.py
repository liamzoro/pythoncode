...
def makeboard() :
    ...
    for but in range(tiles.size) :
        ...
        tiles[but].labelcolor(168)
        tiles[but].labelfont(FL_HELVETICA_BOLD)
        tiles[but].labelsize(size//2 + size//4)
        boxes[but].hide()
...
def showMines() :
    global master,d_master,tiles,d_tiles,boxes,d_boxes
    for t in range(len(master)) :
        if master[t] == 1 :
            if tiles[t].label() is None :
                tiles[t].labelcolor(0)
                tiles[t].label("X")

        elif tiles[t].label() == "F" :
            tiles[t].labelcolor(0)
            tiles[t].label("F!")
    Fl.redraw()
