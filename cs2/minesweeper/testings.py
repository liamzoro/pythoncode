...
class Minesweeper(Fl_Window) :
    ...
    def make_leaderboard(self) :
        self.leaderboard = Fl_Window(256, 512)
        self.leaderboard.begin()
        tabnames = ("Beginner", "Intermediate", "Expert")
        self.records_browser = Fl_Tabs(16, 16, 224, 480)
        self.tabs = []
        self.display = []
        ...
        for i in range(3) :
            self.tabs.append(Fl_Group(16, 48, 224, 480, tabnames[i]))
            self.display.append(Fl_Browser(32, 64, 192, 416))
            self.display[i].textfont(FL_HELVETICA)
            self.display[i].textsize(28)
            self.display[i].align(FL_ALIGN_RIGHT)
    ...
    def make_board(self, size) :
        ...
        self.restart.add("New", FL_CTRL | ord("r"), self.reset_game, self.attributes)
